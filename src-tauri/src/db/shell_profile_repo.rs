...
const SELECT: &str = "select id, name, terminal_exe, shell_exe, shell_args, init_script, is_default from shell_profiles";
...
pub fn save(conn: &Connection, profile: &ShellProfile) -> rusqlite::Result<()> {
    conn.execute(
        "update shell_profiles set name = ?2, terminal_exe = ?3, shell_exe = ?4, \
         shell_args = ?5, init_script = ?6, is_default = ?7 where id = ?1",
        params![
            profile.id,
            profile.name,
