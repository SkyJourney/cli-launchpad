...
- CLI 任务结束后新增独立的“刷新版本中”过渡状态，完成本机版本回读后再恢复最终操作状态。

### 变更

- Claude Code、Codex 和 Antigravity 改为按 CLI 独立维护安装与更新任务；同一 CLI 内互斥，不同 CLI 可并行执行、记录日志和单独终止。
- 设置页按 CLI 独立维护命令计划、确认浮层、创建状态和错误；创建任务后保留在设置页，方便继续触发其他 CLI。
- 安装与更新按钮在获取命令计划时保持原有文案、颜色和尺寸；执行期间仅对应 CLI 显示“安装中”或“更新中”。
- 任务结束后只强制回读本机 CLI 状态，不再重复刷新不会因本机安装而改变的远端最新版本。

### 修复

- 修复点击安装或更新按钮后短暂灰显、文案闪动和按钮变形的问题。
- 修复一个 CLI 执行任务时错误禁用其他 CLI 安装与更新入口的问题。
- 修复更新成功后版本回读期间短暂重新显示旧“有更新”标签和更新按钮的问题。

### 验证

- 已在 macOS 开发版中完成真实 Antigravity 更新、任务完成 Toast、版本回读过渡状态及浅色/深色主题人工验证。
- 前端生产构建、Rust 全量测试和格式检查均通过。

## [0.2.2] - 2026-08-20

### 修复

- 修复 macOS DMG 等生产构建中点击 CLI 安装或更新按钮后确认浮层保持隐藏、界面看似无响应的问题。
- 安装与更新操作现在为 Claude Code、Codex 和 Antigravity 分别使用稳定的浮层锚点，避免生产模式首次定位时引用尚未绑定。

### 验证

- 已在生产前端诊断包和 Release App 中验证更新确认浮层能够正常打开、取消并再次打开，且安装计划 IPC 返回正常。
- 本次验证未执行确认更新，不会改动本机已安装的 CLI。
